...
public class GetMsgService extends Service {
	...
	private BroadcastReceiver backKeyReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			Toast.makeText(context, "HS进入后台运行", 0).show();
			setMsgNotification();
		}
	};
	// 用来更新通知栏消息的handler
			...
			case MSG:
				...
				if (textObject != null) {
					...
					ChatMsgEntity entity = new ChatMsgEntity("",
							MyDate.getDateEN(), content, null, true, Integer.parseInt(util.getId()));// 收到的消息
					messageDB.saveMsg(form, entity, Integer.parseInt(util.getId()));// 保存到数据库
	...
	private void setMsgNotification() {
		...
		Intent intent = new Intent(this, FriendListActy.class);
